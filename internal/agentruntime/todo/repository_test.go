...
func TestInMemoryTodoRepository_AddGeneratesIncrementingIDs(t *testing.T) {
	...
	t1, err := repo.Add(context.Background(), Todo{Content: "task 1", Status: TodoPending, Priority: TodoPriorityHigh})
	if err != nil {
		t.Fatalf("failed to add todo: %v", err)
	}
	t2, err := repo.Add(context.Background(), Todo{Content: "task 2", Status: TodoPending, Priority: TodoPriorityLow})
	if err != nil {
		t.Fatalf("failed to add todo: %v", err)
	}
...
func TestInMemoryTodoRepository_ClearResetsIDCounter(t *testing.T) {
	repo := NewInMemoryTodoRepository()
	_, _ = repo.Add(context.Background(), Todo{Content: "task 1", Status: TodoPending, Priority: TodoPriorityHigh})

	if err := repo.Clear(context.Background()); err != nil {
		t.Fatalf("failed to clear todos: %v", err)
	}

	t1, err := repo.Add(context.Background(), Todo{Content: "task 2", Status: TodoPending, Priority: TodoPriorityLow})
	if err != nil {
		t.Fatalf("failed to add todo: %v", err)
	}
	if t1.ID != "todo-1" {
...
func TestInMemoryTodoRepository_UpdateStatusUnknownReturnsError(t *testing.T) {
	...
	err := repo.UpdateStatus(context.Background(), "todo-404", TodoCompleted)
	if err == nil {
...
func TestInMemoryTodoRepository_UpdateStatusUpdatesExistingItem(t *testing.T) {
	...
	added, err := repo.Add(context.Background(), Todo{Content: "task 1", Status: TodoPending, Priority: TodoPriorityHigh})
	if err != nil {
		t.Fatalf("failed to add todo: %v", err)
	}

	if err := repo.UpdateStatus(context.Background(), added.ID, TodoCompleted); err != nil {
	...
	if todos[0].Status != TodoCompleted {
		t.Fatalf("expected status %q, got %q", TodoCompleted, todos[0].Status)
	}
}
...
func TestInMemoryTodoRepository_ListReturnsAllItems(t *testing.T) {
	repo := NewInMemoryTodoRepository()
	_, _ = repo.Add(context.Background(), Todo{Content: "task 1", Status: TodoPending, Priority: TodoPriorityHigh})
	_, _ = repo.Add(context.Background(), Todo{Content: "task 2", Status: TodoInProgress, Priority: TodoPriorityMedium})
