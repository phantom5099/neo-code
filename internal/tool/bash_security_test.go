...
func (m mockSecurityChecker) Check(_ string, _ string) Action {
	return m.action
}

func TestBashTool_Run_DeniedBySecurity(t *testing.T) {
	SetSecurityChecker(mockSecurityChecker{action: ActionDeny})
...
func TestBashTool_Run_AskBySecurity(t *testing.T) {
	...
	if result.Metadata["securityAction"] != string(ActionAsk) {
		t.Fatalf("unexpected security action: %#v", result.Metadata["securityAction"])
	}
}
