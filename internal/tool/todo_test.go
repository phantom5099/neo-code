...
func TestTodoToolRunAddMissingContentReturnsError(t *testing.T) {
	tool, _ := newTodoTool()
...
func TestTodoToolRunAddInvalidPriorityReturnsError(t *testing.T) {
	tool, _ := newTodoTool()
...
func TestTodoToolRunLifecycle(t *testing.T) {
	...
	if !strings.Contains(listRes.Output, "[/]") {
		t.Fatalf("expected in-progress status icon in output, got %q", listRes.Output)
	}
