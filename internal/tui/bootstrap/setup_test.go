...
func TestPrepareWorkspaceResolvesAndSetsWorkspaceRoot(t *testing.T) {
	...
	if initializedConfigDir != filepath.Join("D:/neo-code/workspace", "internal", "config", "security") {
...
func TestHandleSetupDecisionContinueWriteFailure(t *testing.T) {
	...
	cfg := config.DefaultAppConfig()
	writeAppConfig = func(string, *config.AppConfiguration) error { return errors.New("write failed") }
...
func TestEnsureAPIKeyInteractiveReturnsConfigError(t *testing.T) {
	...
	ensureConfigFile = func(string) (*config.AppConfiguration, bool, error) {
...
func TestEnsureAPIKeyInteractiveWritesConfigAfterSuccessfulValidation(t *testing.T) {
	...
	validateChatAPIKey = func(context.Context, *config.AppConfiguration) error { return nil }
	var writePath string
	writeAppConfig = func(path string, gotCfg *config.AppConfiguration) error {
		writePath = path
		if gotCfg != cfg {
...
func TestEnsureAPIKeyInteractiveReportsCreatedConfigThenSucceeds(t *testing.T) {
	...
	validateChatAPIKey = func(context.Context, *config.AppConfiguration) error { return nil }
	writeAppConfig = func(string, *config.AppConfiguration) error { return nil }
