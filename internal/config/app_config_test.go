...
func TestAppConfigurationValidateMissingMemoryStoragePath(t *testing.T) {
	...
	if err := cfg.Validate(); err != nil {
		t.Fatalf("expected missing storage path to fall back to default, got: %v", err)
	}
	if strings.TrimSpace(cfg.Memory.StoragePath) == "" {
		t.Fatal("expected missing storage path to be defaulted")
	}
}
...
func TestResolvePersonaFilePathFallsBackToCheckedInFile(t *testing.T) {
	...
	if resolved != "./configs/persona.txt" {
		t.Fatalf("expected legacy persona path to resolve to %q, got %q", "./configs/persona.txt", resolved)
	}
